...
class sfPropelCrudGenerator extends sfAdminGenerator{
	 ...
	 * @throws sfException
	 */
	protected function loadTableMapClasses(){
		// we must load all table map classes to be able to deal with foreign keys (cf. editSuccess.php template)
		$classes = sfFinder::type('file')->ignore_version_control()->name('*TableMap.php')->in(sfLoader::getModelDirs());
		foreach($classes as $class){
			$class_table_map = basename($class, '.php');
			$map = Propel::getDatabaseMap()->addTableFromMapClass($class_table_map);
			if(!$map){
				throw new sfException('The table map '.$class_table_map.' can not be instanciated.');
			}

			if($this->className == str_replace('TableMap', '', $class_table_map)){
				$this->map = $map;
			}
		}
		if(!$this->map){
