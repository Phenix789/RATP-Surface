...
class sfPropelData extends sfData
{
   ...
   */
  protected function loadTableMap($class) {
    $tableMapClass = $class . 'TableMap';
    if (!isset($this->maps[$class])) {
      if (!$classPath = sfCore::getClassPath($tableMapClass)) {
        throw new sfException(sprintf('Unable to find path for class "%s".', $tableMapClass));
      }

      require_once($classPath);
      $this->maps[$class] = new $tableMapClass();
      $this->maps[$class]->setDatabaseMap(Propel::getDatabaseMap());
    }
  }
