...
class BasePeer
{
    ...
    public static function doDelete(Criteria $criteria, PropelPDO $con)
    {
        ...
        foreach ($tables as $tableName => $columns) {
            ...
            try {
                ...
                if (!$stmt->execute()) {
					self::debugMSSQL($stmt);
				}
                $affectedRows = $stmt->rowCount();
            } catch (Exception $e) {
                Propel::log($e->getMessage(), Propel::LOG_ERR);
				self::debugSQLParams($sql, $params);
	...
	/**
	 * PATCH DBLIB not considering setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)
	 * @param PDOStatement $stmt
	 * @throws PropelException
	 */
	protected static function debugMSSQL(PDOStatement $stmt){
		$con = Propel::getConnection();
		$err = $stmt->errorInfo();
		$tmp_sql = "select * FROM master.sys.messages m ";
		$tmp_sql .= " INNER JOIN master.dbo.syslanguages l ON l.msglangid = m.language_id";
		$tmp_sql .= " WHERE l.langid = @@LANGID and message_id = ".$err[1];
		$stmt = $con->prepare($tmp_sql);
		if ($stmt->execute()) {
			$row = $stmt->fetch();
			throw new PropelException("{$err[1]}\n<br/>\n{$row['text']}\n");
		} else {
			throw new PropelException($err[2]);
		}
	}

	/**
	 * Simple function to bind params to
	 * @param type $sql
	 * @param array $params
	 */
	protected static function debugSQLParams(&$sql, array $params){
		return;
		$i = 1;
		foreach($params as $value){
			if(isset($value['value'])){
				$sql = str_replace(':p'.$i, "'".$value['value']."'", $sql);
				$i++;
			}
		}
	}
        ...
        if (!empty($orderBy)) {
            ...
            foreach ($orderBy as $orderByColumn) {
                ...
                if ($criteria->isIgnoreCase() && $column && $column->isText()) {
					...
					if(stripos($selectSql, $ignoreCaseColumn) === false){
