...
class sfLoader
{
  ...
  static public function getTemplateDirs($moduleName)
  {
    ...
    if (in_array($moduleName, sfConfig::get('sf_enabled_modules', array())))         // Check if in enabled_modules first (vPatch)
    {
      $pluginDirs = glob(sfConfig::get('sf_plugins_dir').'/*/modules/'.$suffix);
      if($pluginDirs){
        $dirs = array_merge($dirs, $pluginDirs);                                     // plugins
      }
    }
