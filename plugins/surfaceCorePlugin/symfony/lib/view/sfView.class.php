...
abstract class sfView {
	...
	protected function preRenderCheck() {
		if ($this->template == null) {
			...
			$error = 'No template has been set';
		...
		if (!is_readable($template)) {
			...
			$template = SF_ROOT_DIR . '/plugins/surfaceCorePlugin/templates/module_templates/' . $this->template;
			if (!is_readable($template)) {
				throw new sfRenderException("The template '{$this->template}' does not exist in '{$this->directory}' or is not readable");
			}
			$this->directory = SF_ROOT_DIR . '/plugins/surfaceCorePlugin/templates/module_templates';
		}
		...
		if ($this->decorator) {
			...
			if (!is_readable($template)) {
				...
				$template = SF_ROOT_DIR . '/plugins/surfaceCorePlugin/templates/' . $this->decoratorTemplate;
				if (!is_readable($template)) {
					throw new sfRenderException("The decorator template '{$this->decoratorTemplate}' does not exist in '{$this->decoratorDirectory}' or is not readable");
				}
				$this->decoratorDirectory = SF_ROOT_DIR . '/plugins/surfaceCorePlugin/templates';
			}
		}
	}
