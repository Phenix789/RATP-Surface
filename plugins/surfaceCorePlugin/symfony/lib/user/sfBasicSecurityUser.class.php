...
class sfBasicSecurityUser extends sfUser implements sfSecurityUser
{
  ...
  public function initialize($context, $parameters = null)
  {
    ...
    {
      if (null === ($sf_timeout = $this->getAttribute('sf_timeout'))) {
        $sf_timeout = sfConfig::get('sf_timeout', 1480);
      }

      // Automatic logout logged in user if no request within [sf_timeout] setting
      if (0 !== $sf_timeout && null !== $this->lastRequest && (time() - $this->lastRequest) > $sf_timeout)
      {
        if (sfConfig::get('sf_logging_enabled'))
        {
