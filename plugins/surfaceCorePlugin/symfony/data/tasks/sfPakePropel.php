...
$schemas = array_merge(
		pakeFinder::type('file')->ignore_version_control()->name('*schema.xml')->prune('doctrine')->in(glob(sfConfig::get('sf_root_dir').'/plugins/*/config')),
		pakeFinder::type('file')->ignore_version_control()->name('*schema.xml')->prune('doctrine')->in(sfConfig::get('sf_root_dir') . '/config')
  );
  foreach ($schemas as $schema)
  {
    // reset local prefix
    ...
    pake_copy($schema, 'data' . DIRECTORY_SEPARATOR . 'propel'.DIRECTORY_SEPARATOR.$localprefix.basename($schema));
    if ('' === $localprefix)
    {
      pake_remove($schema, '');
    }
