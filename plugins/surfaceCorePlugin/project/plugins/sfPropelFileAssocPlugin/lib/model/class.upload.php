        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if ($this->uploaded) {
					...
					$this->file_src_name_body = preg_replace("/(.*)\.([^.]+)$/","\\1", $this->file_src_name);
					$this->file_src_name_ext = preg_replace("/.*\.([^.]+)$/","\\1", $this->file_src_name);
            ...
            if ($this->uploaded) {
                ...
                preg_match(preg_quote('\.([^\.]*$)'), $this->file_src_name, $extension);
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                ...
                $this->file_dst_name_body = preg_replace('/[^a-zA-Z0-9\s]/', '', $this->file_dst_name_body);
