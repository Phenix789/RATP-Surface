...
function gexpander_remote( $content, $url = '', $update='', $href='', $options = array() )
{
	_loadRessources();
	$html_options = _parse_attributes($options);
	$html_options['class'] = (isset($options['class'] ))?$options['class'] . ' gexpander':'gexpander';
	...
	return link_to_remote($content, array(
			'update' 	=> $update,
			'url'		=> $url,
			'script'	=> true,
			'loading'  => "Element.hide('theNavTrans');"."Element.show('indicator2');", 
  			'complete' => visual_effect('fade', 'indicator2', array('duration' => 0.3)).visual_effect('appear', 'theNavTrans', array('duration' => 0.3))
			), $html_options);
}
...
function gbox_remote($content, $url = '', $update='', $href='' , $options = array())
{
	_loadRessources();
	$html_options = _parse_attributes($options);
	$html_options['class'] = (isset($options['class'] ))?$options['class'] . ' gbox':'gbox';
...
function loadRessourcesgWidgets() {
	_loadRessources();
}
