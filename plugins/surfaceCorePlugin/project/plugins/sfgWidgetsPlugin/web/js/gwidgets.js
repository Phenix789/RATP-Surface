...
var gTab = gWidget.extend({
	...
	constructor: function(elt, tabitem_default) {
		this.elt = elt;
		...
		this.show(this.getInitialTab(tabitem_default));
	},
	...
	getInitialTab: function(tabitem_default) {
		if(document.location.href.match(/#(\w.+)/)) {
			var loc = RegExp.$1;
			var elt = this.menu.find(function(value) { return value.href.match(/#(\w.+)/)[1] == loc; });
			return elt || this.menu.first();
		}
		else if (tabitem_default) {
			var elt = this.menu.find(function(value) { return value.href.match(/#(\w.+)/)[1] == tabitem_default; });
			return elt || this.menu.first();
		}
		else {
...
var gExpander = gWidget.extend({
	...
	constructor: function(elt) {
		...
		else { $$('.' + this.widgetName).each(this.observer.bind(this)); }
	},
...
var gBox = gWidget.extend({
	...
	doModal: function(elt){
		...
		Event.stopObserving(this.idOverlay,'click');
...
function gWidget_InitPane(pane_id){
	if (typeof(gWidget_Options) == 'undefined' || typeof(gWidget_Options.declarative) == 'undefined') {
		new gBox();

		$$("#" + pane_id + ' .gexpander').each(function(elt){
			new gExpander(elt);
		});
		
		$$("#" + pane_id + ' .gtab').each(function(elt){
			new gTab(elt.readAttribute('id'));
		});
		new gTip();
	}
...
function gWidget_InitTab(tab_id, tabitem_default) {
	new gTab(tab_id, tabitem_default);
}
