 ...
 *  - <OpenLayers.Control>
 */
OpenLayers.Control.ElogysGetPointInfo = OpenLayers.Class(OpenLayers.Control, {
	...
	initialize: function(url, target, options) {
		...
		this.EVENT_TYPES =
		OpenLayers.Control.WMSGetFeatureInfo.prototype.EVENT_TYPES.concat(OpenLayers.Control.prototype.EVENT_TYPES);
	...
	request: function(clickPosition, options) {
		...
		lonLat = this.map.getLonLatFromPixel(clickPosition);
		if (lonLat) {
			
			var url = this.url + encodeURI('?layers='+layerNames
			...
			surface.link_to(url, this.target.id, {});
	...
	handleResponse: function(xy, request) {
		if (this.target) {
			this.target.update(request.responseText);
		}
	...
	CLASS_NAME: "OpenLayers.Control.GetPointInfo"
});
