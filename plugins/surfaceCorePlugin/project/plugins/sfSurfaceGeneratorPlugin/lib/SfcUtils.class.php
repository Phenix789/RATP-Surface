...
class SfcUtils {
	...
	public static function convertToText($html, $len = null) {
		...
		return $this->summarize(html_entity_decode(strip_tags($html)), $len);
	}
	...
	/**
	 * Test the existence of a key in an array and return it
	 * @param mixed $key
	 * @param array $array
	 * @param mixed $default
	 * @return mixed $value
	 */
	public static function getKey($key, array $array, $default = null) {
		if (is_array($key)) {
			$tab = $array;
			$count = count($key);
			for ($i = 0; $i < $count; $i++) {
