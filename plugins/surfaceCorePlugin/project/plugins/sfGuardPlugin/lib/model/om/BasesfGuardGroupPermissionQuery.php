 ...
 * @method     sfGuardGroupPermissionQuery orderByEntityId($order = Criteria::ASC) Order by the entity_id column
 *
 ...
 * @method     sfGuardGroupPermissionQuery groupByEntityId() Group by the entity_id column
 *
 ...
 *
 * @method     sfGuardGroupPermissionQuery leftJoinsfGuardEntity($relationAlias = null) Adds a LEFT JOIN clause to the query using the sfGuardEntity relation
 * @method     sfGuardGroupPermissionQuery rightJoinsfGuardEntity($relationAlias = null) Adds a RIGHT JOIN clause to the query using the sfGuardEntity relation
 * @method     sfGuardGroupPermissionQuery innerJoinsfGuardEntity($relationAlias = null) Adds a INNER JOIN clause to the query using the sfGuardEntity relation
 *
 ...
 * @method     sfGuardGroupPermission findOneByEntityId(int $entity_id) Return the first sfGuardGroupPermission filtered by the entity_id column
 *
 ...
 * @method     array findByEntityId(int $entity_id) Return sfGuardGroupPermission objects filtered by the entity_id column
 *
...
abstract class BasesfGuardGroupPermissionQuery extends ModelCriteria
{
     ...
     * @param array $key Primary key to use for the query 
                         A Primary key composition: [$group_id, $permission_id, $entity_id]
    ...
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = sfGuardGroupPermissionPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1], (string) $key[2]))))) && !$this->formatter) {
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            sfGuardGroupPermissionPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1], (string) $key[2])));
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        $this->addUsingAlias(sfGuardGroupPermissionPeer::ENTITY_ID, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(sfGuardGroupPermissionPeer::ENTITY_ID, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
    ...
    {
        if (is_array($entityId) && null === $comparison) {
            $comparison = Criteria::IN;
        }

        return $this->addUsingAlias(sfGuardGroupPermissionPeer::ENTITY_ID, $entityId, $comparison);
    }
    ...
    /**
     * Filter the query by a related sfGuardEntity object
     *
     * @param   sfGuardEntity|PropelObjectCollection $sfGuardEntity The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   sfGuardGroupPermissionQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterBysfGuardEntity($sfGuardEntity, $comparison = null)
    {
        if ($sfGuardEntity instanceof sfGuardEntity) {
            return $this
                ->addUsingAlias(sfGuardGroupPermissionPeer::ENTITY_ID, $sfGuardEntity->getId(), $comparison);
        } elseif ($sfGuardEntity instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(sfGuardGroupPermissionPeer::ENTITY_ID, $sfGuardEntity->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterBysfGuardEntity() only accepts arguments of type sfGuardEntity or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the sfGuardEntity relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return sfGuardGroupPermissionQuery The current query, for fluid interface
     */
    public function joinsfGuardEntity($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('sfGuardEntity');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'sfGuardEntity');
        }
    ...
    /**
     * Use the sfGuardEntity relation sfGuardEntity object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   sfGuardEntityQuery A secondary query class using the current class as primary query
     */
    public function usesfGuardEntityQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinsfGuardEntity($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'sfGuardEntity', 'sfGuardEntityQuery');
    }
    ...
    public function prune($sfGuardGroupPermission = null)
    {
        if ($sfGuardGroupPermission) {
            ...
            $this->addCond('pruneCond2', $this->getAliasedColName(sfGuardGroupPermissionPeer::ENTITY_ID), $sfGuardGroupPermission->getEntityId(), Criteria::NOT_EQUAL);
