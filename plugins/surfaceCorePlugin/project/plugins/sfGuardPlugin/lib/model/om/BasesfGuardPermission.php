 ...
 */
abstract class BasesfGuardPermission extends BaseObject 
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {				
				$stmt = $con->query('SELECT sf_guard_permission_SEQ.nextval FROM dual');
				$row = $stmt->fetch(PDO::FETCH_NUM);
				$this->id = $row[0];
            } catch (Exception $e) {
     ...
     * @return PropelObjectCollection|sfGuardGroupPermission[] List of sfGuardGroupPermission objects
     */
    public function getsfGuardGroupPermissionsJoinsfGuardEntity($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = sfGuardGroupPermissionQuery::create(null, $criteria);
  ...
  try {
    ...
    {
      throw new sfException(sprintf('Call to undefined method BasesfGuardPermission::%s', $name));
    }
  ...
  */
  ...
  public function exportTo($parser, $includeLazyLoadColumns = true) {
	...
	public function getMetadata($info = null, $default = null) {
		return sfGuardPermissionPeer::getMetadata($info, $default, get_class($this));
	}
