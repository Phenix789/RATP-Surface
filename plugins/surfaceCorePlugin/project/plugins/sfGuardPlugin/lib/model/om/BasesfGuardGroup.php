 ...
 */
abstract class BasesfGuardGroup extends BaseObject 
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {				
				$stmt = $con->query('SELECT sf_guard_group_SEQ.nextval FROM dual');
				$row = $stmt->fetch(PDO::FETCH_NUM);
				$this->id = $row[0];
            } catch (Exception $e) {
  ...
  try {
    ...
    {
      throw new sfException(sprintf('Call to undefined method BasesfGuardGroup::%s', $name));
    }
  ...
  */
  ...
  public function exportTo($parser, $includeLazyLoadColumns = true) {
	...
	public function getMetadata($info = null, $default = null) {
		return sfGuardGroupPeer::getMetadata($info, $default, get_class($this));
	}
