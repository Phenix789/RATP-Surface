...
class PluginsfGuardUser extends BasesfGuardUser {
	...
	public function getProfileName() {
		...
		if($profile && method_exists($profile, 'getUsername')) {
	...
	public function hasPermissionInAll($name) {
	...
	public function getPermissions() {
		if(!$this->permissions) {
			...
			foreach($ups as $up) {
				$entity = $up->getsfGuardEntity();
				$permission = $up->getsfGuardPermission();

				if($entity && $permission)
					$this->permissions[$entity->getFullName() . sfGuardEntity::ENTITY_SEPARATOR . $permission->getName()] = $permission;
				else {
					if($permission)
						$this->permissions[sfGuardEntity::ENTITY_SEPARATOR . $permission->getName()] = $permission;
					else
						$this->permissions[$entity->getFullName() . sfGuardEntity::ENTITY_SEPARATOR] = null;
				}
			}
		}
		...
		if(!$this->allPermissions) {
			...
			foreach($this->getGroups() as $group) {
				foreach($group->getsfGuardGroupPermissions() as $gp) {
					$entity = $gp->getsfGuardEntity();
					$permission = $gp->getsfGuardPermission();

					if($entity && $permission)
						$this->allPermissions[$entity->getFullName() . sfGuardEntity::ENTITY_SEPARATOR . $permission->getName()] = $permission;
					else {
						if($permission)
							$this->allPermissions[sfGuardEntity::ENTITY_SEPARATOR . $permission->getName()] = $permission;
						else
							$this->allPermissions[$entity->getFullName() . sfGuardEntity::ENTITY_SEPARATOR] = null;
					}
				}
			}
		}
