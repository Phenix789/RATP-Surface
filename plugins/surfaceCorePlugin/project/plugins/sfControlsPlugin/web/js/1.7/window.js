...
UI.WindowManager = Class.create(UI.Options, {
  ...
  */
  ...
  setAltitude: function(win, altitude) {
    ...
    if (altitude === "front") {
      if (stack.getFrontWindow() === win) return false;
      stack.bringToFront(win);
    } else if (altitude === "back") {
      if (stack.getBackWindow() === win) return false;
      stack.sendToBack(win);
    } else {
      if (stack.getPosition(win) == altitude) return false;
...
UI.Splitter = Class.create(UI.Options, {
  options: {
      container:          null, // will default to document.body
      pane1:              null,
      pane2:              null,
      ...
      sizable:            true,
      pane1Collapsable:   true,
      pane2Collapsable:   true,
      theme:              ""
  },
  ...
  destroy: function() {
    ...
    this.delSplitterButton("collapse_left");
    this.delSplitterButton("collapse_rigth");
    this.delSplitterButton("collapse_restore");
  },
  ...
  create: function() {
    ...
    this.element =  $(this.options.container); /* createDiv("ui-window " /* + this.getTheme() * /, {
      id: this.options.id,
      style: "top:-10000px; left:-10000px"
    });
    */

    this.element.addClassName('splitter ' + this.options.theme);
    
    this.pane1      = createDiv('pane');
    this.pane1.update($(this.options.pane1));
    // this.pane1 = $(this.options.pane1);
    this.splitter   = createDiv('splitbar'); // .insert("test");
    this.pane2      = createDiv('pane');
    this.pane2.update($(this.options.pane2));
   
    this.element.insert(this.pane1);
    this.element.insert(this.splitter);
    this.element.insert(this.pane2);
    
    this.splitter.setStyle(  { zIndex: this.element.getStyle('zIndex') + 100  } );
    this.pane1.setStyle(  { zIndex: this.element.getStyle('zIndex')  } );
    this.pane2.setStyle(  { zIndex: this.element.getStyle('zIndex')  } );
    
    this.setBarOrientation(this.options.splitHorizontal);
    this.resize(true, '50%');
    this.setSizable(this.options.sizable);
    this.setCollapsable(this.options.pane1Collapsable, this.options.pane2Collapsable);
  
    this.element.observe('drag:started', this.onStartDrag.bind(this));
    this.element.observe('drag:updated', this.onDrag.bind(this));
    this.element.observe('drag:ended',   this.onEndDrag.bind(this));
    
    this.resizeEvent = this.onResize.bind(this);
    this.panelResizeEvent = this.onPanelResize.bind(this);
    Event.observe(window, "resize", this.resizeEvent);
    this.element.observe("panel:resize", this.panelResizeEvent);
  },
  
  onStartDrag: function(event) {
    event.stop();
    this.initDragBounds = { left: parseInt(this.splitter.style.left) - this.offset[0], top: parseInt(this.splitter.style.top) - this.offset[1]  };
  },
  onDrag: function(event) {
    event.stop();
    this.splitter.addClassName('resized');
    this.drag(event.memo.dx, event.memo.dy);
  },
  onEndDrag: function(event) {
    event.stop();
    this.splitter.removeClassName('resized');
    this.drag(event.memo.dx, event.memo.dy);
  },
  
  drag: function(dx, dy) {
    this.setSplitterPos(this.options.splitHorizontal? (this.initDragBounds.top + dy) : (this.initDragBounds.left + dx));
  },
  ...
  onPanelResize: function(event) {
    if (event.memo.window == this.element) {
      event.stop();
      this.resize();
    }
  },
  
  resize: function(bNoNotify, newSplitterPos) {
    var pos;
    
    this.barWidth = this.options.splitHorizontal? parseInt(this.splitter.getHeight()) : parseInt(this.splitter.getWidth());
    
    if (newSplitterPos)
      pos = newSplitterPos;
    else {
      pos = (this.getSplitterPos() * 100) / ((this.options.splitHorizontal? parseInt(this.dimensions.height) : parseInt(this.dimensions.width)) - this.barWidth);
      pos = Math.round(pos) + '%';
    }
    
    this.offset = this.element.getOffsetParent();
    this.offset[0] = 0; this.offset[1] = 0;
    
    this.dimensions = this.element.getDimensions();
    this.dimensions.width =  parseInt(this.dimensions.width) - this.offset[0];
    this.dimensions.height = parseInt(this.dimensions.height) - this.offset[1];
         
    if (this.options.splitHorizontal) {
      this.setChildBounds(this.splitter,  { left:   this.offset[0] + 'px',
                                            right:  '0px' } );
      this.setChildBounds(this.pane1,  {  left:   this.offset[0] + 'px',
                                          right:  '0px' } );
      this.setChildBounds(this.pane2,  {  left:   this.offset[0] + 'px',
                                          right:  '0px' } );
    }
    else {
      this.setChildBounds(this.splitter,  { top:    this.offset[1] + 'px',
                                            bottom: '0px'  } );
      this.setChildBounds(this.pane1,  {  top:    this.offset[1] + 'px',
                                          bottom: '0px' } );
      this.setChildBounds(this.pane2,  {  top:    this.offset[1] + 'px',
                                          bottom: '0px' } );
       
    }
    
    this.setSplitterPos(pos, !this.notifying);
  },
  
  setSplitterPos: function(pos, bNoNotify) {
      if ((typeof pos == 'string') && (pos.indexOf('%') != -1)) {
        pos = (parseInt(pos) * ((this.options.splitHorizontal? parseInt(this.dimensions.height) : parseInt(this.dimensions.width)) - this.barWidth)) / 100;
        pos = Math.floor(pos);
      }
      
      if (parseInt(pos) < 0) {
        pos = (this.options.splitHorizontal? parseInt(this.dimensions.height) : parseInt(this.dimensions.width)) + parseInt(pos);
      }

      if (this.options.splitHorizontal) {
        if (parseInt(pos) < 0)
          pos = 0;
        else if (parseInt(pos) > (parseInt(this.dimensions.height) - this.barWidth))
          pos = parseInt(this.dimensions.height) - this.barWidth;
          
        realpos = parseInt(pos) + this.offset[1] ;
        this.setChildBounds(this.pane1,     { top:  this.offset[1] + 'px',              bottom:  (parseInt(this.dimensions.height) - parseInt(pos)) + 'px'   } );
        this.setChildBounds(this.splitter,  { top:  realpos + 'px'                      } );
        this.setChildBounds(this.pane2,     { top:  (realpos + this.barWidth) + 'px',   bottom:  '0px'    } );
      }
      else {
        if (parseInt(pos) < 0)
          pos = 0;
        else if (parseInt(pos) > (parseInt(this.dimensions.width) - this.barWidth))
          pos = parseInt(this.dimensions.width) - this.barWidth;

        realpos = parseInt(pos) + this.offset[0];
        this.setChildBounds(this.pane1,      { left:  this.offset[0] + 'px',              right:  (parseInt(this.dimensions.width) - parseInt(pos)) + 'px'   } );
        this.setChildBounds(this.splitter,   { left:  realpos + 'px'                      } );
        this.setChildBounds(this.pane2,      { left:  (realpos + this.barWidth) + 'px',   right:  '0px'    } );
      }
      
      if (!bNoNotify) {
          memo = { };
          memo.window = this.pane1.firstDescendant();
          this.pane1.firstDescendant().fire("panel:resize", memo);
          
          memo.window = this.pane2.firstDescendant();          
          this.pane2.firstDescendant().fire("panel:resize", memo);
      }
  },
  
  getSplitterPos: function() {
    return (this.options.splitHorizontal)? (parseInt(this.splitter.getStyle('top')) - parseInt(this.offset[1])) : (parseInt(this.splitter.getStyle('left')) - parseInt(this.offset[0]));
  },
  
  setChildBounds: function (element, bounds) {
    if (Prototype.Browser.IE6) {
      /* ie 6 ne comprends pas 'bottom: 0px' ou 'right: 0px' ! */
      var tmp = {};
      
      if (bounds.top)
        tmp.top = bounds.top;
      if (bounds.left)
        tmp.left = bounds.left;
      if (bounds.bottom)
        tmp.height = (parseInt(this.dimensions.height) - parseInt(bounds.bottom) - ((tmp.top)? parseInt(tmp.top) : 0)) + 'px';
      if (bounds.right)
        tmp.width = (parseInt(this.dimensions.width) - parseInt(bounds.right) - ((tmp.left)? parseInt(tmp.left) : 0)) + 'px';
      
      element.setStyle(tmp);
    }
    else
      element.setStyle(bounds);
  },
  
  setSizable: function(sizable) {
    this.options.sizable = sizable;
    if (this.options.sizable) {
      this.splitter.enableDrag();
      this.splitter.addClassName("sizable"); 
    }
    else {
      this.splitter.disableDrag();
      this.splitter.removeClassName("sizable");
    }
  },
  
  setBarOrientation: function(bHorizontal) {
    this.options.splitHorizontal = bHorizontal;
    if (this.options.splitHorizontal) {
      this.splitter.removeClassName("verticalbar");
      this.splitter.addClassName("horizontalbar");
    }
    else {
      this.splitter.removeClassName("horizontalbar");
      this.splitter.addClassName("verticalbar");
    }
  },
  
  collapsePane: function(bSecondPane) {
    this.collapsePos = this.options.splitHorizontal? (parseInt(this.splitter.getStyle('top')) - this.offset[1]) : (parseInt(this.splitter.getStyle('left')) - this.offset[0]);
    if (bSecondPane) {
      this.setSplitterPos((this.options.splitHorizontal? parseInt(this.dimensions.height) : parseInt(this.dimensions.width)) - this.barWidth - 1);
    }
    else {
      this.setSplitterPos(0);
    }
    
    this.showSplitterButton("collapse_pane1",   false);
    this.showSplitterButton("collapse_pane2",   false);
    this.showSplitterButton("collapse_restore", true);
    var button = this.getSplitterButton("collapse_restore");
    button.removeClassName(bSecondPane? "pane1" : "pane2");
    button.addClassName(bSecondPane? "pane2" : "pane1");

    // On désactive de re-taillement de panneaux
    this.wasSizable = this.options.sizable;
    if (this.options.sizable) {
  ...
  restorePane: function() {
    this.setSplitterPos(this.collapsePos);
    
    this.showSplitterButton("collapse_restore", false);
    this.showSplitterButton("collapse_pane1",   true);
    this.showSplitterButton("collapse_pane2",   true);

    if (this.wasSizable) {
      this.setSizable(this.wasSizable);
    }
  },
  
  setCollapsable: function(bFirstPane, bSecondPane) {
    this.options.pane1Collapsable = bFirstPane;
    if (this.options.pane1Collapsable)
      this.addSplitterButton("collapse_pane1", this.onCollapsePane1);
    else 
      this.delSplitterButton("collapse_pane1");
        
    this.options.pane2Collapsable = bSecondPane;
    if (this.options.pane2Collapsable)
      this.addSplitterButton("collapse_pane2", this.onCollapsePane2);
    else
      this.delSplitterButton("collapse_pane2");
    
    if (this.options.pane1Collapsable || this.options.pane2Collapsable)
      this.addSplitterButton("collapse_restore", this.onRestorePane).hide();
    else
      this.delSplitterButton("collapse_restore");
  },
  
  addSplitterButton: function(classname, action) {
      button = new Element("a", { className: classname + (this.options.splitHorizontal? "_h" : "_v"), href: "#"});
      this.splitter.insert(button);
      button.setStyle(  { zIndex: this.splitter.getStyle('zIndex') + 1  } );
      button.observe('click', action.bind(this))
      return button;
  },
  
  getSplitterButton: function(classname) {
    return this.splitter.down("." + classname + (this.options.splitHorizontal? "_h" : "_v"));
  },
  
  delSplitterButton: function(classname) {
      button = getSplitterButton(classname);
      if (button) {
  ...
  showSplitterButton: function(classname, bVisible) {
    button = this.getSplitterButton(classname)
    if (button) {
      bVisible? button.show() : button.hide();
    }  
  },
  
  onCollapsePane1: function(event) {
    this.collapsePane(false);
  },
  
  onCollapsePane2: function(event) {
    this.collapsePane(true);
  },
  
  onRestorePane: function(event) {
    this.restorePane();
   }
});
