...
Control.ColorPicker.prototype = {
	initialize: function (element, options) {
		this.element = Element.extend($(element));
		this.options = Object.extend({
				classNamePanel: 'colorpickerControlPanel',
				classNameContainer: 'colorpickerControl',
				classNameInput: 'colorpickerControlInput',
				classNamePicker: 'colorpickerControlPicker'	//inputExtension
		...
		cpCont.className = this.options.classNamePanel;
		cpCont.appendChild(this.colorpicker.element);
		this.dialog.appendChild(cpCont);

		var cont = new Element('span', {'class': this.options.classNameContainer });
		this.element.parentNode.replaceChild(cont, this.element);
		cont.appendChild(this.element);
		this.element.addClassName(this.options.classNameInput);
		...
		// Element.setStyle(this.swatch, { 'left': left, 'top': top, 'width': size + 'px', 'height': size + 'px', 'backgroundColor': this.element.value});
		Element.setStyle(this.swatch, { 'backgroundColor': this.element.value});
		this.swatch.title = 'Palette';
		if (this.element.id)
		   this.swatch.id = this.element.id + '_swatch';
		this.swatch.className = this.options.classNamePicker;
...
Control.ColorPickerPanel.prototype = {
	...
	initialize: function(options) {
		this.options = Object.extend({
				addLabel: 'Add',
				colors: Array(
					'#B0CC99', '#677E52', '#F6E8B1', '#89725B', '#A7A37E', '#EFECCA', '#046380', '#002F2F',
					'#FF5B2B', '#B1221C', '#34393E', '#8CC6D7', '#C44C51', '#FFB6B8', '#A2B5BF', '#5F8CA3',
